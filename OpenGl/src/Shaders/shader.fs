...
struct Material {
    vec3 ambientReflectance;   // 环境光反射系数（物体对环境光的反射能力）
    vec3 diffuseReflectance;   // 漫反射系数（物体对直射光的颜色和强度）决定物体的主色调（如红色物体吸收蓝绿光，反射红光）。
    vec3 specularReflectance;  // 镜面反射系数（高光的颜色和强度） 通常与物体材质的真实颜色无关（如金属的高光颜色接近金属本身，塑料的高光多为白色）。
...
void main()
{
    ...
    vec3 ambient = light.ambient * material.ambientReflectance;//环境光
    vec3 specular = light.specular * material.specularReflectance * spec ;// 镜面反射分量
    vec3 diffuse = light.diffuse * (diff * material.diffuseReflectance);// 漫反射分量
