...
vec3 CalcDirLight(dirLight light, vec3 normal, vec3 viewDir){//方向(平行)光计算
    ...
    // 合并结果
    vec3 ambient  = light.ambient  * vec3(texture(material.diffuseTextureID, TexCoord));//环境光
    vec3 diffuse  = light.diffuse  * diff * vec3(texture(material.diffuseTextureID, TexCoord));//漫反射
    vec3 specular = light.specular * spec * vec3(texture(material.specularTextureID, TexCoord));//镜面反射
